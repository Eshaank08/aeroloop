...
interface UseCommandRunnerOptions {
  ...
  /** Called with the operator's words once a run is accepted, to hand off to mission control. */
  onLaunch?: (command: string) => void
}

export function useCommandRunner({ initialEntries, onLaunch }: UseCommandRunnerOptions) {
  ...
  const start = useCallback(() => {
    ...
    onLaunch?.(command)
    return true
  }, [commandText, isRunning, onLaunch])
